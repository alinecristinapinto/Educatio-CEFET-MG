...
//percorre tabela decarte e coloca os valores em array
			while($row = $result->fetch_assoc()){
				...
				$valorRetornado .= "<tr><td>".$idAcervo."</td><td>".$nomeacervo."</td><td>".$tipoacervo."</td><td>".$localacervo."</td><td></tr>";
			}
		...
		while($linha = mysqli_fetch_array($result)){
			...
			if (stripos($linha["2"],VALOR_RECEBIDO) !== FALSE) {
				if ($valorRetornado === "") {
					$valorRetornado = "<thead><tr><th>id</th><th>Nome</th><th>tipo</th><th>Local</th><th>editora</th><th>ano</th></tr></thead><tbody style = 'cursor: pointer;'>";
					$valorRetornado .= "<tr onclick=\"enviaFormulario('" .$linha[0] ."')\"><td>$linha[0]</td><td>$linha[2]</td><td>$linha[3]</td><td>$linha[4]</td><td>$linha[6]</td><td>$linha[5]</td></tr>";
				} else {
					$valorRetornado .= "<tr onclick=\"enviaFormulario('" .$linha[0] ."')\"><td>$linha[0]</td><td>$linha[2]</td><td>$linha[3]</td><td>$linha[4]</td><td>$linha[6]</td><td>$linha[5]</td></tr>";
				}
			}
