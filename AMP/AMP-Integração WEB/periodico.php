...
class Periodico{
	...
	function deletaPeriodico($StringDeterminante, $StringDeterminado){
        ...
        $sqlIdPeriodico = " SELECT id FROM periodicos WHERE $StringDeterminante  = '$StringDeterminado'";

		$resultatoQueryidPeriodico = $sqlConexao->query($sqlIdPeriodico);

		$vetorResultado = $resultadoQueryidPeriodico->fetch_assoc();

		$intIdPeriodico = $vetorResultado["id"];

		if (mysqli_query($sqlConexao, $sqlIdPeriodico)) {	
		...
		$deletandoPartes = " UPDATE partes SET ativo = 'N' WHERE id = '$intIdPeriodico'";
		if (mysqli_query($sqlConexao, $deletandoPartes)) {
			echo "Acervo deletado!<br>";
		}
		else{
			echo "Erro ao deletar o acervo: " . mysqli_error($sqlConexao);
    ...
    function criaParte($stringTitulo, $stringPaginicio, $stringPagfinal, $stringPalavraschave){
        ...
        $sqlIdPeriodico = "SELECT  MAX(id) FROM acervo";

		$result = mysqli_query($sqlConexao, $sqlIdPeriodico);

		$intIdperiodico = mysqli_fetch_array($result);
        
        $sqlInsereParte = "INSERT INTO partes (idPeriodico, titulo, pagInicio, pagFinal, palavrasChave, ativo) VALUES ('$intIdperiodico[0]','$stringTitulo','$stringPaginicio', '$stringPagfinal', '$stringPalavraschave', 'S')";
		if ($result = mysqli_query($sqlConexao, $sqlInserePeriodico)) {			
		}else{
			echo "Erro: " . $sqlInserePeriodico . "<br>" . $sqlConexao->error;
		}
...
$aaa -> criaPeriodico($_POST["periodicidade"],$_POST["mes"],$_POST["volume"],$_POST["subtipo"],$_POST["ISSN"],'S','1',$_POST["nome"],$_POST["tipo"],$_POST["local"],$_POST["ano"],$_POST["editora"],$_POST["paginas"]);
