...
###############################################################################################
    ...
    echo "<script>
            var res = confirm('Você está preste a executar uma ação que mudará o ano de todos os alunos aprovando ou reprovando estes de acordo com o rendimento obtido por eles durante o ano. Clique em OK se deseja continuar.');
            if (res == false){
                location.href = 'gerencia-web-perfil-coordenador.php';
            }
          </script>"
    // TESTANDO A CONEXÃO
...
###############################################################################################
    function confereDiario()
    {
        ...
        while($linhaCursos = $rstCursos->fetch_assoc()) 
        {
            ...
            while ($linhaTurmas = $rstTurmas->fetch_assoc()) {
                ...
                {
                    echo "<br><b><h4>".$linhaAlunos['nome']."</h4></b>";
    ...
    // echo "<script>
    //         alert('Atualização de ano dos alunos feita com sucesso!');
    //       </script>";
?>