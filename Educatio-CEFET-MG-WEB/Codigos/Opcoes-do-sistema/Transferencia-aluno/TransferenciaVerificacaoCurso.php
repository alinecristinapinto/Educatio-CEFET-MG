<?php
	...
	$strDepto = $_GET['strDepto'];
	$strSQL = $conn->query("SELECT id, nome FROM `Educatio`.`deptos` WHERE nome ='".$strDepto."'");
		...
		<div class="col-md-8 ml-auto mr-auto">
			...
			<form method="POST" action="" class="contact-form">
				...
				<div class="input-group">
					...
					<input type="text" name="Nome" id="txt_consulta" placeholder="Digite o Nome do Curso onde está o Aluno" class="form-control" required='required'>
				</div>							
			
				<table class="table table-hover" required='required' id="tabela">
					<!-- Pega os dados do banco e coloca na tabela -->
					<?php $strSQL = $conn->query("SELECT id, nome, ativo FROM `Educatio`.`cursos` WHERE idDepto = '".$intIdDepto."'");
					while($arrLinha = $strSQL->fetch_assoc()) {
						if ($arrLinha['ativo'] != 'N') {	
							echo "<tr value='".$arrLinha['nome']."' onclick('document.getElementById('txt_consulta').value = document.getElementById(this).innerHTML')><th>".$arrLinha['nome']."</th></tr>";
						}
					}?>
				</table>
...
<?php
	if (isset($_POST["Selecionar"])){
		echo "<script>
					location.href = 'TransferenciaVerificacaoTurma.php?strCurso=".$_POST['Nome']."';
				</script>";
		//header('Location: TransferenciaVerificacaoTurma.php?strCurso='.$_POST['Nome']);
	}
